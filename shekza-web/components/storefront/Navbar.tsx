...
const categories = [
  { label: 'Fashion', icon: '👗', href: '/products?category=fashion' },
  { label: 'Shoes', icon: '👟', href: '/products?category=shoes' },
  { label: 'Bags', icon: '👜', href: '/products?category=bags' },
  { label: 'Beauty', icon: '💄', href: '/products?category=beauty' },
  { label: 'Electronics', icon: '📱', href: '/products?category=electronics' },
  { label: 'Home', icon: '🏠', href: '/products?category=home' },
];

const suggestions = ['Pastel Hoodie', 'Platform Sneakers', 'Mini Crossbody Bag', 'Rose Gold Watch', 'Aesthetic Room Decor'];

export default function Navbar({ cartCount = 0, wishlistCount = 0 }: { cartCount?: number; wishlistCount?: number }) {
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <div className="flex items-center gap-4">
              ...
              <Link href="/" className="flex items-center gap-2 pr-4 md:border-r md:border-neutral-100 mr-2">
                ...
                >
                  <Zap size={16} className="text-white fill-white md:w-18 md:h-18" />
                </motion.div>
                <span className="text-xl md:text-2xl font-black text-neutral-900 tracking-tight">
                  Shek<span className="text-pink-500">za</span>
                </span>
              </Link>
                    ...
                    <div className="bg-white/95 backdrop-blur-xl rounded-[2rem] shadow-2xl shadow-black/10 border border-neutral-100 p-6 min-w-[500px] grid grid-cols-2 gap-4">
                      {categories.map((cat) => (
                        ...
                        >
                          ...
                          <div>
                            <h4 className="text-sm font-black text-neutral-900 group-hover/item:text-pink-500 transition-colors">{cat.label}</h4>
                    ...
                    >
                      <div className="p-6 bg-gradient-to-br from-neutral-50 to-pink-50/30">
                        ...
                        <h4 className="text-lg font-black text-neutral-900 leading-tight">
                          {mounted ? (isLoggedIn ? `Hello, ${user?.fullName || 'User'} ✨` : 'Join shekza 🌸') : '...'}
                        </h4>
                        {!isLoggedIn && (
              ...
              >
                <div className="p-6 border-b border-neutral-100 flex items-center justify-between">
                  <Link href="/" className="flex items-center gap-2">
                    ...
                    <span className="text-lg font-black italic">shekza</span>
                  </Link>
                ...
                <div className="flex-grow overflow-y-auto p-6 space-y-8">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {categories.map(cat => (
                        <Link key={cat.label} href={cat.href} className="flex flex-col items-center gap-2 p-4 bg-neutral-50 rounded-2xl hover:bg-pink-50 transition-colors text-center border border-neutral-100">
                          <span className="text-2xl">{cat.icon}</span>
                          <span className="text-xs font-black uppercase truncate w-full">{cat.label}</span>
                        </Link>
                      ))}
                    </div>
                  </div>
                ...
                <div className="p-6 bg-neutral-50 border-t border-neutral-100">
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-neutral-900">help@shekza.id</p>
                    </div>
                  </div>
                </div>
              </motion.div>
