      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-neutral-500 text-sm font-medium">#{order.orderNumber || 'N/A'} · {order.createdAt ? new Date(order.createdAt).toLocaleDateString() : 'N/A'}</p>
          </div>
          <motion.div
            ...
            <div className="bg-white rounded-[2.5rem] p-8 border border-neutral-100 shadow-xl shadow-black/5">
              ...
              <div className="relative pl-1 px-4">
                ...
                <div className="space-y-12">
                  {[
                    { label: 'Order Created', completed: true, date: order.createdAt ? new Date(order.createdAt).toLocaleDateString() : 'N/A' },
          ...
          <div className="lg:col-span-1 space-y-8">
            <div className="bg-neutral-900 rounded-[2.5rem] p-8 text-white sticky top-24">
              ...
              <div className="space-y-6 mb-8">
                {order.items.map((item) => (
                  <div key={item.id} className="flex gap-4 group">
                    ...
                    <span className="text-sm font-black text-white">Rp {item.price.toLocaleString('id-ID')}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="pt-6 border-t border-white/10 flex justify-between items-end">
                ...
                <span className="text-2xl font-black text-white">Rp {order.totalAmount?.toLocaleString('id-ID')}</span>
              </div>
            </div>
          </div>
        </div>
